...
st.markdown(
    "<h1 style='margin-top: -80px; text-align: center;'>Prasa w mediach społecznościowych</h1>",
...
def barplot(df, column):
    ...
    for index, value in enumerate(list(df)):
        if value > 0:
            plt.text(value + margin, index + 0.1, format_number_with_spaces(value))
    ...
    for index, pismo in enumerate(list(df.index)):
        plt.text(
            ...
            ha="left",
            va="center",
            fontdict={"fontsize": 10.8, "fontname": "Lato"},
        )
    return fig
...
def barplot_suma(filtered_df):
    ...
    for bar, value in zip(bars, top10["Suma_Selected"]):
        plt.text(
            bar.get_x() + bar.get_width() + margin,
...
def create_donut(donutdf):
    ...
    label = [
        medium
        + ": "
        + str(round(100 * sumdict[medium] / sumdict["Suma"], 1)).replace(".", ",")
        + "%"
...
st.markdown("---")
st.markdown(
    "<h2 style='text-align: center; font-size: 1.27em;'>Udział poszczególnych platform</h2>",
    unsafe_allow_html=True,
)
...
st.markdown("---")
st.markdown(
    "<h2 style='text-align: center; font-size: 1.27em;'>Liczba obserwatorów w social mediach</h2>",
    unsafe_allow_html=True,
)
...
st.markdown(
    '<p style="font-size: 14px;">Wybierz media społecznościowe:</p>',
    unsafe_allow_html=True,
)
...
if output_type == "Tabela":
    ...
    filtered_df = filtered_df[
        filtered_df.index.str.contains(searchbar, case=False, na=False)
    ]
    ...
    filtered_df_html = filtered_df_html.replace(
        "<table", "<table class='sticky-header'"
    )

    st.markdown(table_css_style, unsafe_allow_html=True)
    st.markdown(
        f"<div class='sticky-table'>{filtered_df_html}</div>", unsafe_allow_html=True
    )

else:
    if selected_typ == ["Dodatki"]:
        st.write(
            "Proszę wybrać więcej grup pism, kategoria jest zbyt wąska, by wyświetlać wykresy."
        )
        st.stop()
...
footer_message = "Źródło: Liczba obserwatorów w mediach społecznościowych, opracowanie własne PBC, dane na dzień 07.01.2026"
st.markdown(
